...
import com.app.kiyalearning.databinding.StudentFragmentHomeBinding
import com.app.kiyalearning.student.dashboard.DashBoardActivity
import com.app.kiyalearning.student.dashboard.ui.classes.adapters.AdapterStudentSibling
import com.app.kiyalearning.student.dashboard.ui.home.pojos.DashBoardData
...
class HomeFragment : Fragment(), AdapterStudentSibling.AdapterStudentSiblingListener {

    private var _binding: StudentFragmentHomeBinding? = null
    private lateinit var viewModel: HomeViewModel
    private var siblingList = mutableListOf<StudentSibilingResponse>()
...
// onDestroyView.
    ...
    private fun setClickListeners() {
        ...
        binding.cancelClassesCard.setOnClickListener {
            val activity = activity as DashBoardActivity
            activity.classesSelectedTab = 2
            activity.binding!!.navView.selectedItemId = R.id.navigation_classes
        }

        binding.classesTakenCard.setOnClickListener {
            val activity = activity as DashBoardActivity
            activity.classesSelectedTab = 1
            activity.binding!!.navView.selectedItemId = R.id.navigation_classes
        }
        ...
        binding.assessmentCard.setOnClickListener {
            val intent = Intent(requireActivity(), AssessmentActivity::class.java)
            startActivity(intent)
        }

        binding.testSeriesCard.setOnClickListener {
            val intent = Intent(requireActivity(), TestSeriesActivity::class.java)
            startActivity(intent)
        }
    ...
    private fun setData(data: DashBoardData) {
        ...
        binding.userName.text = AppPref.getUserName(requireContext())
        ...
        setSiblingData(data.sibilings as MutableList<StudentSibilingResponse>)
...
//        binding.due.text=data.due
    ...
    private fun setSiblingData(list: MutableList<StudentSibilingResponse>) {
        siblingList.clear()
        siblingList.addAll(list)

        binding.rvSibling.apply {
            layoutManager =
                LinearLayoutManager(requireContext(), LinearLayoutManager.HORIZONTAL, false)
            adapter = AdapterStudentSibling(siblingList, this@HomeFragment)
        }
    }
    ...
    fun showReportDialog(
        ...
        dB.apply {
            ...
            etClassName.setOnClickListener {
                try {
                    showDatePickerDialog(it.context, object : DatePickerDialogListener {
            ...
            tvSendBtn.setOnClickListener {
                ...
                Log.d(
                    "MyTag",
                    "Issue : Usertype Student Issue $issue and comment $comment className  $className"
                )

                binding.loader.pB.visibility = View.VISIBLE

                viewModel.addStudentTechnicalReportStatus(
                    requireContext(),
                    "Student",
                    className,
                    issue,
    ...
    override fun onClickStudentSibling(m: StudentSibilingResponse) {
     //   binding.loader.pB.visibility = View.VISIBLE
        viewModel.loginWithSiblingId(requireContext(), m.id.toString())
        viewModel.loginWithSiblingIdResponse.observe(requireActivity(), Observer {
            if (it.success){
                AppPref.updateUserData(requireContext(),it)
                binding.loader.pB.visibility = View.VISIBLE
                viewModel.getDashBoardData(requireActivity())
