...
import com.app.kiyalearning.teacher.dashboard.ui.home.pojos.DashBoardDataResponse
...
class HomeViewModel : ViewModel() {
    ...
    var addTechnicalReportStatus =
        MutableLiveData<AcceptRejectResponse>()
    ...
    fun getNotificationsList(context: Context) {
        ...
        val call: Call<NotificationsResponse> = RestManager.getInstance().getNotifications(
    ...
    fun getDashBoardData(context: Context) {
        val map = HashMap<String, String>()
        //  map["consumption_id"] = consumptionId
        val call: Call<DashBoardDataResponse> = RestManager.getInstance().getTeacherDashBoardData(
            AppPref.getTokenType(context) + " " + AppPref.getUserToken(context)
        )
        call.enqueue(object : Callback<DashBoardDataResponse> {
            override fun onResponse(
                call: Call<DashBoardDataResponse>, response: Response<DashBoardDataResponse>
            ) {
                if (response.isSuccessful) {
            ...
            override fun onFailure(call: Call<DashBoardDataResponse>, t: Throwable) {
                Log.d("MyTag", "onFailure: Dashboard " + t.message)
                validationError.value = context.getString(R.string.server_error)
    ...
    fun addTeacherTechnicalReportStatus(
        context: Context, userType: String, className: String, issue: String, comment: String
    ) {
        ...
        call.enqueue(object : Callback<AcceptRejectResponse> {
            override fun onResponse(
                call: Call<AcceptRejectResponse>, response: Response<AcceptRejectResponse>
            ) {
                if (response.isSuccessful) {
